...
namespace Quran.Core.Data
{
    public class AyahInfoDatabaseHandler : BaseDatabaseHandler
    {
        ...
        public Rectangle GetPageBounds(int page)
        {
            AyahLimits limits = dbConnection.Table<AyahLimits>().FirstOrDefault(a => a.PageNumber == page);
            if (limits == null) return null;
            var r = new Rectangle(limits.MinX, limits.MinY, limits.MaxX, limits.MaxY);
            return r;
        }

        public QuranAyah GetVerseAtPoint(int page, double x, double y)
        {
            var ayahBounds = dbConnection.Table<AyahBounds>().Where(a => a.PageNumber == page);
        ...
        {
            var allAyahBounds = dbConnection.Table<AyahBounds>().Where(a => a.PageNumber == page && a.Line == line);
