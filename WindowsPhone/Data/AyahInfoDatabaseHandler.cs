...
namespace QuranPhone.Data
{
    public class AyahInfoDatabaseHandler : BaseDatabaseHandler
    {
        ...
        {
            IList<AyahBounds> bounds = GetVerseBounds(sura, ayah);
            ...
            if ((first != null) && (current != null) && (first.Position != current.Position))
            {
                last = current;
            }
        ...
        private IList<AyahBounds> doHighlightAyah(AyahBounds first, AyahBounds last,
            Dictionary<int, AyahBounds> lineCoordinates)
        {
            if (first == null)
            {
                return null;
        ...
        {
            TableQuery<AyahBounds> ayahBounds = MDatabase.Query<AyahBounds>().Where(a => a.PageNumber == page);
            if (!ayahBounds.Any())
            {
                return null;
            }
            ...
            foreach (AyahBounds bound in ayahBounds)
            {
        ...
        {
            TableQuery<AyahBounds> allAyahBounds =
                MDatabase.Query<AyahBounds>().Where(a => a.PageNumber == page && a.Line == line);
            if (!allAyahBounds.Any())
            {
                return null;
            }
            ...
            foreach (AyahBounds bound in allAyahBounds)
            {
            ...
            foreach (int ayahNumber in ayahs.Keys)
            {
