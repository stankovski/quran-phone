...
using System;
using System.IO;
using QuranPhone.SharpZipLib.BZip2;

namespace QuranPhone.SharpZipLib.BZip2 {
		...
		public static void Decompress(Stream inStream, Stream outStream, bool isStreamOwner) 
		{
			...
			try {
				using (BZip2InputStream bzipInput = new BZip2InputStream(inStream)) {
					...
					QuranPhone.SharpZipLib.Core.StreamUtils.Copy(bzipInput, outStream, new byte[4096]);
				}
			} finally {
				if (isStreamOwner) {
		...
		public static void Compress(Stream inStream, Stream outStream, bool isStreamOwner, int level) 
		{
			...
			try {
				using (BZip2OutputStream bzipOutput = new BZip2OutputStream(outStream, level)) {
					...
					QuranPhone.SharpZipLib.Core.StreamUtils.Copy(inStream, bzipOutput, new byte[4096]);
				}
			} finally {
				if (isStreamOwner) {
