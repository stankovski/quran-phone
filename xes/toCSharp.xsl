...
<xsl:apply-templates select="*[(self::importList) or (self::comment)]"/>
<xsl:param name = "packageName" select="@name"></xsl:param>
<xsl:if test="string-length($packageName) &gt; 0">
namespace <xsl:value-of select="@name"/> {
</xsl:if>
<xsl:apply-templates select="*[not(self::importList) and not(self::comment)]"/>
<xsl:if test="string-length($packageName) &gt; 0">
}
</xsl:if>
</xsl:template>
...
using <xsl:value-of select="@name"/>;</xsl:template>

<xsl:template match="classDef">
<xsl:text>
</xsl:text>
<xsl:if test="@new">new<xsl:text> </xsl:text></xsl:if>
...
<xsl:if test="@internal">internal<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@static">static<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@abstract">abstract<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@sealed">sealed<xsl:text> </xsl:text></xsl:if>
...
<xsl:if test="@abstract">abstract<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@sealed">sealed<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@virtual">virtual<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@override">override<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@extern">extern<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@unsafe">unsafe<xsl:text> </xsl:text></xsl:if>
...
<xsl:choose>
<xsl:when test="@elementName = 'implements'"><xsl:text> : </xsl:text></xsl:when>
<xsl:otherwise>
<xsl:text> </xsl:text><xsl:value-of select="@elementName"/></xsl:otherwise>
</xsl:choose>
...
<xsl:if test="@internal">internal<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@static">static<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@readonly">readonly<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@volatile">sealed<xsl:text> </xsl:text></xsl:if>
