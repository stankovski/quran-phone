...
<xsl:if test="@native">native<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@synchronized">synchronized<xsl:text> </xsl:text></xsl:if>
<xsl:if test="@strictfp">strictfp<xsl:text> </xsl:text></xsl:if>
...
<xsl:if test="parent::block or parent::method or parent::classDef">; <!-- ; if directly in a class, method or block --> 
</xsl:if>
</xsl:template>
...
if(<xsl:apply-templates select="*[1]"/>)<xsl:apply-templates select="*[not(position()) = 1]"/>
</xsl:template>
...
<xsl:if test="not(@postOp = 'true')"><xsl:value-of select="@operator"/></xsl:if>
<xsl:apply-templates select="*"/><xsl:if test="@postOp = 'true'"><xsl:value-of select="@operator"/></xsl:if><xsl:if test="parent::block or parent::method or parent::classDef">; <!-- ; if directly in a class, method or block -->
</xsl:if>
</xsl:template>
