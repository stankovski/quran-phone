...
namespace Quran.UniversalApp.NativeProvider
{
    public class WindowsDownloadManager : IDownloadManager
    {
        ...
        {
            var request = new WindowsWebClientTransferRequest(serverUri, phoneUri)
            {
        ...
        public ITransferRequest DownloadMultipleAsync(string[] from, string to)
        {
            ...
            var request = new WindowsMultifileTransferRequest(from, phoneUri)
            {
                RequestId = Guid.NewGuid().ToString()
            };
            request.Download();
        ...
        {
            return BackgroundTransferService.Requests.Select(r => new WindowsTransferRequest(r));
        }
